...
async def get_latest_image() -> Response:
    """Return the most recently sent image as JPEG, or 404 if none yet."""
...
async def get_latest_meta() -> dict:
    ...
    Returns ``{"sent_at": "HH:MM:SS", "filename": <name>, "success": <bool>}``,
    with all fields null when nothing has been sent yet.
    """
    latest = await producer_state.get()
...
async def list_records() -> dict:
    ...
    Returns ``{"records": [...]}``; each record also carries ``added_at`` and
    ``send_success``.
    """
